...
bool DL_Dxf::getStrippedLine(std::string& s, unsigned int size, FILE *fp, bool stripSpace) {
	...
	/////////////////////////////////////////////////////////////////////////////////
	//dl_dxf 会读取出所有图元数据，包括  ENTITIES  段之外的。
	// 但 ENTITIES 段之外的数据， 本程序并不需要，    所以添加这段代码，判断是否进入 ENTITIES 段
	static bool InENTITIES = false;
	if (line&&strstr(line, "ENTITIES")) {	//进入实体
		InENTITIES = true;
	}

	if (line&&InENTITIES&&strstr(line, "ENDSEC")) {//退出实体
		InENTITIES = false;
	}
	if (!InENTITIES) {
		s = "";

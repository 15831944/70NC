...
struct DXFLIB_EXPORT DL_VertexData {
    ...
    bool operator == (const DL_VertexData& data) {

	    bool b1, b2, b3, b0;
	    b0 = this->x == data.x&&this->y == data.y&&this->z == data.z;
	    b1 = abs(this->x - data.x) <= 0.0001;
	    b2 = abs(this->y - data.y) <= 0.0001;
	    b3 = abs(this->z - data.z) <= 0.0001;
	    return b1&&b2&&b3;
    }
    bool operator = (const DL_VertexData& data) {
	    this->x = data.x; 
...
struct DXFLIB_EXPORT DL_LineData {
	...
	*/
	...
	DL_VertexData   GetPoint1() {
		return DL_VertexData(x1, y1, z1);
	}

	DL_VertexData   GetPoint2() {
		return DL_VertexData(x2, y2, z2);
	}
...
struct DXFLIB_EXPORT DL_ArcData {
	...
	*/
	...
	DL_VertexData   GetPoint1() {
		double pi = 3.1415926;
		double x1, y1;
		x1 = cx + radius*cos(pi*angle1 / 180);
		y1 = cy + radius*sin(pi*angle1 / 180);
		return DL_VertexData(x1, y1, cz);
	}

	DL_VertexData   GetPoint2() {
		double pi = 3.1415926;
		double x2, y2;
		x2 = cx + radius*cos(pi*angle2 / 180);
		y2 = cy + radius*sin(pi*angle2 / 180);
		return DL_VertexData(x2, y2, cz);
	}
